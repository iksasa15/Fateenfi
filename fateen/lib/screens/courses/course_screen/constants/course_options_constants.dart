// course_options_constants.dart

class CourseOptionsConstants {
  // عناوين الخيارات
  final String editCourseTitle = 'تعديل المقرر';
  final String filesTitle = 'ملفات المقرر';
  final String gradesTitle = 'الدرجات';
  final String notificationsTitle = 'الإشعارات';

  // أزرار
  final String deleteButtonText = 'حذف المقرر';
  final String closeButtonText = 'إغلاق';

  // نص تأكيد الحذف
  final String deleteConfirmTitle = 'تأكيد الحذف';
  final String deleteConfirmMessage =
      'هل أنت متأكد من رغبتك في حذف هذا المقرر؟ لا يمكن التراجع عن هذه العملية.';
  final String confirmDelete = 'حذف';
  final String cancelDelete = 'إلغاء';

  // رسائل النجاح والخطأ
  final String deleteSuccess = 'تم حذف المقرر بنجاح';
  final String deleteError = 'حدث خطأ أثناء حذف المقرر';
  final String notificationSuccess = 'تم جدولة الإشعار بنجاح';
  final String notificationError = 'حدث خطأ أثناء جدولة الإشعار';
}
